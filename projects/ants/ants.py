...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if not (isinstance(insect, QueenAnt) and insect.first):
                insect.place = None
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while curr_place is not hive:
            if curr_place.bees and self.min_range<=dist<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if not self.place and curr_place.bees:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.first:
            ...
            while p.exit:
                if p.exit.ant:
                    if p.exit.ant.is_container and p.exit.ant.contained_ant:
                        if p.exit.ant.doubled == False:
                            p.exit.ant.damage = 2 * p.exit.ant.damage
                            p.exit.ant.doubled = True
                        if p.exit.ant.contained_ant.doubled == False:
                            p.exit.ant.contained_ant.damage = 2 * p.exit.ant.contained_ant.damage
                            p.exit.ant.contained_ant.doubled = True
                    else:
                        if not p.exit.ant.doubled:
                            p.exit.ant.damage = 2 * p.exit.ant.damage
                            p.exit.ant.doubled = True
                p = p.exit
        else:
    ...
    def result(colony):
        ...
        if effect == make_scare:
            ...
            if duration == 0:
                ...
                duration = duration * (bee.slowed + 1)
                new(colony)
                duration -=1
    bee.action = result
    # END Problem EC
