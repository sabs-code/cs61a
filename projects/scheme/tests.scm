...
(define sum-square (lambda (x y) (* (+ x y) (+ x y))))
(sum-square x y)
;expect 36

(or (and 2 (< 2 0)) (or (* 2 3) 9))
;expect 6
...
(begin (display 'no) (* 2 7) (or (< 0 1) (> 0 1)) #f)
;expect no#f

((lambda (x y) (* (* x x) (* y y))) 3 2)
;expect 36

((lambda (f x y) (f x y)) square-product x y)
;expect 25

(define x ((lambda (f x y) (f x y)) square-product x y))
;expect x
...
((< x 25) 'wrong)
((> x 25) 'alsowrong)
((= x 25) (begin (display 'no) (* 2 7) (or (< 0 1) (> 0 1)) #f)))
;expect no#f

(define square-product (mu () (* (* a a) (* b b))))
(lambda (1 2) square-product)
;expect Error
