...
def taxicab(a, b):
    ...
    9
    """
    street0, street1 = street(a), street(b)
    ave0, ave1 = avenue(a), avenue(b)
    return abs(street0-street1) + abs(ave0-ave1)
...
def flatten(lst):
    ...
    def helper(a, lst):
        if a >=len(lst):
            return []
        if type(lst[a])==int:
            return [lst[a]] + helper(a+1, lst)
        else:
            return helper(0, lst[a]) + helper(a+1, lst)
    return helper(0, lst)
...
def balanced(m):
    ...
    False
    """
    a = [total_weight(end(x)) for x in branches(m)]
    b = [length(x) for x in branches(m)]
    if a[0]*b[0]==a[1]*b[1]:
        return True
    else:
        return False
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(total_weight(end(left(m))) +total_weight(end(right(m))), b)
